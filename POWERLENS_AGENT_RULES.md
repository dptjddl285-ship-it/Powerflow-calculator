...
## 22. Progress Reporting & User Communication Protocol

작업 중 사용자가 침묵이나 멈춤으로 불안감을 느끼지 않도록 다음 원칙을 매 응답마다 엄격히 준수한다:

1. **상태 태그 및 아이콘 의무 표기**:
   - ⏳ `[진행 중]`: 자율 작업 수행 중
   - ⚙️ `[백엔드/빌드 실행 중]`: 백그라운드 프로세스/빌드/테스트/CDP 브라우저 실행 중
   - 🛑 `[작업 완료 / 사용자 대기]`: 모든 작업 완료 후 대기 중
   - ⚠️ `[블로커 / 확인 필요]`: 사용자 승인 및 추가 데이터 대기 중

2. **정량적 진행률 카운터 (N개 중 M개 완료)**:
   - 다단계 작업 시 `[3/5 완료] (60%)`와 같이 완료 수량 및 상세 체크리스트를 표기한다.

3. **백그라운드 프로세스 투명 안내**:
   - 터미널이나 서버가 백그라운드에서 동작 중일 때 어떤 명령이 몇 초 동안 연산 중인지 명확히 브리핑한다.
