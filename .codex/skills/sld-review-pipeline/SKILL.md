...
* **도구**: 기존 `analyze_circuit_image` 객체 검출 레이어 (`backend_api/core/vision_logic.py`).
...
### 활용 도구
* `backend_api/core/pipeline_policy.py`: `CandidatePolicy.decide()` (ACCEPT, RESCUE, REVIEW, REJECT 판정)
* `backend_api/core/vision_logic.py`: `_refine_generator_circle_for_validation()`, `_looks_like_transformer_pair()`, `_recover_yolo_bus_raster_run()` 등 국소 검증기
...
* **실행 절차**:
  ...
  3. 1픽셀 갭 브릿징 및 스켈레톤화 (`backend_api/core/electrical_topology.py`)
  4. 컴포넌트 포트 생성 및 Endpoint-Port 매칭
  5. 교차로(Crossing) 가상 레인 분기 생성
  6. 방향성 Dijkstra 최단경로 결선 추적 (`backend_api/core/electrical_topology.py`)

---

## Phase E — Connection Review (Agent 결선 검증)

결선 결과를 임의 확정하지 않고 `backend_api/core/pipeline_policy.py`의 `validate_graph()`를 실행하여 전기적 위상 결함을 검출한다.
...
* **결과**: 검증 완료된 단선도가 Flutter `frontend_app/lib/main.dart`의 `PowerCanvasPage`에 오차 없이 정확하게 렌더링된다.
