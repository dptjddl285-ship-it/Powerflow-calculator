...
class ReviewNodeItem {
  ...
  factory ReviewNodeItem.fromJson(Map<String, dynamic> json) {
    ...
    if (json['review_reasons'] is List) {
      reasons = (json['review_reasons'] as List).map((e) => e.toString()).toList();
    }
    ...
    return ReviewNodeItem(
      ...
      reviewStatus: json['review_status']?.toString().toUpperCase() ?? 'DETECTED',
      reviewReasons: reasons,
      agentExplanation: json['agent_explanation']?.toString(),
      recommendedAction: json['recommended_action']?.toString(),
      evidence: json['evidence'] is Map<String, dynamic> ? json['evidence'] : {},
...
class MissingCandidateItem {
  ...
  factory MissingCandidateItem.fromJson(Map<String, dynamic> json) {
    ...
    if (json['approximate_region'] is List) {
      region = (json['approximate_region'] as List).map((e) => (e as num).toDouble()).toList();
    }
...
class CompletenessReviewResult {
  ...
  factory CompletenessReviewResult.fromJson(Map<String, dynamic> json) {
    ...
    var parsed = rawList.map((c) => MissingCandidateItem.fromJson(c as Map<String, dynamic>)).toList();
...
class ReviewLineItem {
  ...
  factory ReviewLineItem.fromJson(Map<String, dynamic> json) {
    ...
    if (rawPath is List) {
      for (var pt in rawPath) {
        if (pt is List && pt.length >= 2) {
          parsedPath.add([(pt[0] as num).toDouble(), (pt[1] as num).toDouble()]);
        }
      }
    }
    ...
    if (rawIssues is List) {
      parsedIssues = rawIssues.map((i) => i is Map<String, dynamic> ? i : <String, dynamic>{}).toList();
    }

    var rawCandidates = json['candidate_connections'] ?? json['candidate_targets'];
...
class PriorityReviewItem {
  ...
  final String targetType; // 'NODE', 'LINE', 'MISSING_CANDIDATE', 'TOPOLOGY_ISSUE'
  final String reason;
...
class ProactiveSummaryItem {
  ...
  factory ProactiveSummaryItem.fromJson(Map<String, dynamic> json) {
    ...
    var parsedItems = rawItems.map((p) => PriorityReviewItem.fromJson(p as Map<String, dynamic>)).toList();
...
class ReviewDocument {
  ...
  factory ReviewDocument.fromJson(Map<String, dynamic> json, {Uint8List? rawBytes}) {
    var rawNodes = json['nodes'] as List? ?? [];
    var parsedNodes = rawNodes.map((n) => ReviewNodeItem.fromJson(n as Map<String, dynamic>)).toList();

    var rawLines = json['lines'] as List? ?? [];
    var parsedLines = rawLines.map((l) => ReviewLineItem.fromJson(l as Map<String, dynamic>)).toList();
    ...
    return ReviewDocument(
      ...
      image: ReviewImageMeta.fromJson(json['image'] as Map<String, dynamic>? ?? {}),
      nodes: parsedNodes,
      lines: parsedLines,
