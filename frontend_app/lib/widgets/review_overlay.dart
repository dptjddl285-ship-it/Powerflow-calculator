...
class ReviewOverlayView extends StatefulWidget {
  ...
  final Function(ReviewNodeItem startNode, ReviewNodeItem endNode)? onManualAddLineComplete;
...
class _ReviewOverlayViewState extends State<ReviewOverlayView> with SingleTickerProviderStateMixin {
  Offset? _dragStart;
  Offset? _dragCurrent;
  ...
  Color _getClassColor(String className) {
    ...
    if (cls.contains('gen')) return const Color(0xFF00E676); // Vibrant Green/Teal
  ...
  ReviewLineItem? _findLineNearTap(Offset tapPos, double scaleX, double scaleY) {
    ...
    for (var line in widget.lines) {
      ...
      for (int i = 0; i < line.path.length - 1; i++) {
        ...
        final b = Offset(line.path[i + 1][0] * scaleX, line.path[i + 1][1] * scaleY);
  ...
  Widget build(BuildContext context) {
    if (widget.imageBytes == null && (widget.imageUrl == null || widget.imageUrl!.isEmpty)) {
      ...
      builder: (context, _) {
        ...
        return LayoutBuilder(
          builder: (context, constraints) {
            ...
            final double origW = widget.originalWidth > 0 ? widget.originalWidth.toDouble() : 1280.0;
            final double origH = widget.originalHeight > 0 ? widget.originalHeight.toDouble() : 720.0;
            ...
            return Center(
              child: SizedBox(
                ...
                child: GestureDetector(
                  ...
                  onTapUp: (details) {
                    if (!widget.isManualAddMode && !widget.isManualAddLineMode) {
                      final line = _findLineNearTap(details.localPosition, scaleX, scaleY);
                      if (line != null) {
                  ...
                  onPanEnd: (details) {
                    if (widget.isManualAddMode && _dragStart != null && _dragCurrent != null) {
                      final double left = math.min(_dragStart!.dx, _dragCurrent!.dx);
                      final double top = math.min(_dragStart!.dy, _dragCurrent!.dy);
                      final double w = (_dragStart!.dx - _dragCurrent!.dx).abs();
                      final double h = (_dragStart!.dy - _dragCurrent!.dy).abs();
                      ...
                      if (w > 10 && h > 10) {
                        ...
                        widget.onManualAddComplete?.call(
                          [origCx, origCy, origWidth, origHeight],
                  ...
                  child: Stack(
                    ...
                    children: [
                      ...
                      ...widget.lines.map((line) {
                        if (line.path.length < 2) return const SizedBox.shrink();
                        ...
                        final double posY = baseMidY - 12 + line.labelOffsetDy;

                        final bool isLineSelected = widget.selectedLine?.lineId == line.lineId;
                        final bool isAmbiguous = line.reviewStatus.toUpperCase() == 'AMBIGUOUS';
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  color: isLineSelected
                                      ? Colors.yellowAccent
                                      : (isAmbiguous ? const Color(0xFFFFD600) : const Color(0xFF1E1E2E).withValues(alpha: 0.85)),
                                  borderRadius: BorderRadius.circular(3),
                                  border: Border.all(
                                    color: isLineSelected ? Colors.orangeAccent : (isAmbiguous ? Colors.redAccent : Colors.cyanAccent),
                                    width: isLineSelected ? 1.5 : 1.0,
                                  ),
                                  boxShadow: const [
                                ...
                                child: Text(
                                  ...
                                  style: TextStyle(
                                    color: (isLineSelected || isAmbiguous) ? Colors.black87 : Colors.cyanAccent,
                                    fontSize: 9,
                      ...
                      ...widget.nodes.map((node) {
                        ...
                        final bool isSelected = widget.selectedNode?.id == node.id;
                        final bool isEndpointOfSelectedLine = lineEndpointNodeIds.contains(node.id);
                        final bool isManualStart = widget.manualLineStartNode?.id == node.id;
                        final isHumanAdded = node.source.contains('human');
                        final isRejected = node.reviewStatus.toUpperCase() == 'REJECTED';

                        final classColor = _getClassColor(node.className);
                        final statusColor = _getStatusColor(node.reviewStatus);

                        final borderColor = isSelected
                            ? Colors.yellowAccent
                            : (isEndpointOfSelectedLine
                                ? Colors.cyanAccent
                                : (isManualStart ? Colors.purpleAccent : statusColor));

                        final double borderWidth = isSelected
                            ? (2.8 + pulseVal * 1.5)
                            : (isEndpointOfSelectedLine ? 2.4 : (isManualStart ? 3.0 : 1.8));

                        // Base label coordinates (can be offset by user drag)
                        final double labelBaseX = renderLeft + node.labelOffsetDx;
                        final double labelBaseY = renderTop - 22 + node.labelOffsetDy;
                              ...
                              child: GestureDetector(
                                onTap: () {
                                  if (widget.isManualAddLineMode) {
                                    ...
                                    } else if (widget.manualLineStartNode!.id != node.id) {
                                      widget.onManualAddLineComplete?.call(widget.manualLineStartNode!, node);
                                    }
                                  } else {
                                    widget.onSelectNode(node);
                                  }
                                ...
                                child: Container(
                                  decoration: BoxDecoration(
                                    ...
                                    boxShadow: (isSelected || isEndpointOfSelectedLine)
                                        ? [
                                            BoxShadow(
                                              color: (isSelected ? Colors.yellowAccent : Colors.cyanAccent).withValues(alpha: 0.3 + pulseVal * 0.4),
                                              blurRadius: 10 + pulseVal * 6,
                                              spreadRadius: 2 + pulseVal * 2,
                                            ),
                                          ]
                                        : null,
                                    color: classColor.withValues(alpha: isSelected ? (0.25 + pulseVal * 0.15) : 0.12),
                                    borderRadius: BorderRadius.circular(3),
                                  ),
                                  child: isRejected
                                      ? const Center(child: Icon(Icons.close, color: Colors.redAccent, size: 18))
                                      : null,
                                ),
                              ),
                            ),
                                    ...
                                    child: Row(
                                      ...
                                      children: [
                                        Icon(
                                          ...
                                          size: 11,
                                          color: (isSelected || isEndpointOfSelectedLine) ? Colors.black87 : Colors.white,
                                        ),
                                        ...
                                        Text(
                                          ...
                                          style: TextStyle(
                                            color: (isSelected || isEndpointOfSelectedLine) ? Colors.black54 : Colors.white70,
                                            fontSize: 8.5,
                                          ),
                                        ),
                                        if (isHumanAdded) ...[
                                          ...
                                          Container(
                                            ...
                                            child: const Text("HUMAN", style: TextStyle(color: Colors.white, fontSize: 8, fontWeight: FontWeight.bold)),
                                          ),
                                        ],
                                        if (node.reviewStatus == 'CONFIRMED') ...[
                                          const SizedBox(width: 2),
                                          const Icon(Icons.check_circle, size: 10, color: Colors.greenAccent),
                                        ] else if (node.reviewStatus == 'SUSPICIOUS') ...[
                                          const SizedBox(width: 2),
                      ...
                      if (_dragStart != null && _dragCurrent != null)
                        Positioned(
                          ...
                          height: (_dragCurrent!.dy - _dragCurrent!.dy).abs(),
                          child: Container(
                            decoration: BoxDecoration(
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var line in lines) {
      if (line.labelOffsetDx.abs() > 6 || line.labelOffsetDy.abs() > 6) {
        ...
        canvas.drawLine(Offset(baseMidX, baseMidY), Offset(labelX, labelY), leaderPaint);
      }
    }
  }
...
class _ConnectionOverlayPainter extends CustomPainter {
  ...
  @override
  void paint(Canvas canvas, Size size) {
    final bool hasSelection = selectedLine != null;
    ...
    for (var line in lines) {
      ...
      if (isSelected) {
        ...
        strokeColor = const Color(0xFF00E676); // Green
        strokeWidth = 2.4;
      } else if (status == 'AMBIGUOUS') {
        strokeColor = const Color(0xFFFFD600); // Amber/Yellow
        strokeWidth = 3.0;
      } else if (status == 'REJECTED') {
        strokeColor = Colors.grey.withValues(alpha: 0.4);
        strokeWidth = 1.5;
      } else {
        strokeColor = const Color(0xFF00E5FF); // Cyan
        strokeWidth = 2.0;
      }

      // Dim non-selected lines when a line is active
      ...
      if (isSelected) {
        final glowPaint = Paint()
          ..color = Colors.yellowAccent.withValues(alpha: 0.3 + pulseValue * 0.4)
      ...
      final startPt = Offset(line.path.first[0] * scaleX, line.path.first[1] * scaleY);
      final endPt = Offset(line.path.last[0] * scaleX, line.path.last[1] * scaleY);

      canvas.drawCircle(startPt, isSelected ? (4.0 + pulseValue * 1.5) : 3.0, dotPaint);
      canvas.drawCircle(endPt, isSelected ? (4.0 + pulseValue * 1.5) : 3.0, dotPaint);
    }
  }
