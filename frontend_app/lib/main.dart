...
class PowerCanvasPageState extends State<PowerCanvasPage>
  ...
  void _handleKeyEvent(KeyEvent event) {
    ...
    final isEditingInput =
        focusedWidget != null && focusedWidget != _canvasFocusNode;
    ...
    if (event.logicalKey == LogicalKeyboardKey.delete &&
      ...
      setState(() {
        selectedElement!.angle =
            (selectedElement!.angle + math.pi / 2) % (math.pi * 2);
      });
      return;
          ...
          if (gens.containsKey(bKey)) {
            ...
            final genEl =
                DrawingElement(
                    ...
                    label:
                        "G_$bNum" +
                        (gInfo['is_slack'] == true ? " (Slack)" : ""),
                  )
                  ..isSlack = (gInfo['is_slack'] == true)
                  ..vPu = (gInfo['voltage_setpoint'] as num?)?.toDouble() ?? 1.0
                  ..pPu = (gInfo['is_slack'] == true)
                      ? 0.0
                      : ((gInfo['pg_pu'] as num?)?.toDouble() ?? 0.0)
                  ..qPu = (gInfo['is_slack'] == true)
                      ? 0.0
                      : ((gInfo['qg_pu'] as num?)?.toDouble() ?? 0.0);
            elements.add(genEl);
          }
    ...
    setState(() {
      ...
      for (var line in rawLines) {
        ...
        if (rawPath.length >= 2) {
          Offset startPos = Offset(
            (rawPath.first[0] as num).toDouble() + shiftX,
            (rawPath.first[1] as num).toDouble() + shiftY,
          );
          Offset endPos = Offset(
            (rawPath.last[0] as num).toDouble() + shiftX,
            (rawPath.last[1] as num).toDouble() + shiftY,
          );

          Offset midPos = rawPath.length > 2
              ? Offset(
                  (rawPath[(rawPath.length / 2).floor()][0] as num).toDouble() +
                      shiftX,
                  (rawPath[(rawPath.length / 2).floor()][1] as num).toDouble() +
                      shiftY,
                )
              : Offset(
          ...
          elements.add(
            DrawingElement(
              ...
              startElementId: connectedTo.isNotEmpty
                  ? connectedTo[0].toString()
                  : null,
              endElementId: connectedTo.length > 1
                  ? connectedTo[1].toString()
                  : null,
            ),
          );
        }
      ...
      for (var el in elements.where((e) => e.type == Tool.load)) {
        if (el.angle == 0.0) {
          ...
          if (connLine != null &&
              connLine.aiPath != null &&
              connLine.aiPath!.isNotEmpty) {
            ...
            if (dx.abs() > dy.abs()) {
              el.angle = dx > 0
                  ? -math.pi / 2
                  : math.pi / 2; // Pointing Right vs Left
            } else {
      ...
      for (var dev in elements.where(
        (e) => e.type == Tool.generator || e.type == Tool.load,
      )) {
  ...
  Widget _buildBusGenLoadWidget(DrawingElement e) {
    ...
    final double leftOffset =
        e.position.dx - (e.width / 2) - (isSelected ? pad : 0);
    final double topOffset =
        e.position.dy - (e.height / 2) - (isSelected ? pad : 0);
              ...
              Positioned(
                ...
                child: GestureDetector(
                  ...
                  onPanUpdate: (d) {
                    setState(() {
                      double deltaX =
                          d.delta.dx * math.cos(e.angle) +
                          d.delta.dy * math.sin(e.angle);
