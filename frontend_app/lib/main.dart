...
class PowerDesignerApp extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => const MaterialApp(home: PowerCanvasPage(), debugShowCheckedModeBanner: false);
}
...
class DrawingElement {
  ...
  double pPu = 0.0; double qPu = 0.0; double rPu = 0.01; double xPu = 0.05;
  ...
  DrawingElement copy() {
    ...
    ..pPu = pPu..qPu = qPu..rPu = rPu..xPu = xPu;
  }
...
class PowerCanvasPageState extends State<PowerCanvasPage> {
  ...
  void _applyAiDataToCanvas(Map<String, dynamic> aiData) {
    ...
    setState(() {
      ...
      for (var line in aiData['lines']) {
        ...
        if (rawPath.length >= 2) {
          Offset startPos = Offset(rawPath.first[0].toDouble() + CANVAS_CENTER + offsetMargin, rawPath.first[1].toDouble() + CANVAS_CENTER + offsetMargin);
          Offset endPos = Offset(rawPath.last[0].toDouble() + CANVAS_CENTER + offsetMargin, rawPath.last[1].toDouble() + CANVAS_CENTER + offsetMargin);
          
          Offset midPos = rawPath.length > 2 
              ? Offset(rawPath[(rawPath.length/2).floor()][0].toDouble() + CANVAS_CENTER + offsetMargin, rawPath[(rawPath.length/2).floor()][1].toDouble() + CANVAS_CENTER + offsetMargin)
              : Offset((startPos.dx + endPos.dx)/2, (startPos.dy + endPos.dy)/2);

          List<Offset> parsedPath = [];
          for (var pt in rawPath) {
            parsedPath.add(Offset(pt[0].toDouble() + CANVAS_CENTER + offsetMargin, pt[1].toDouble() + CANVAS_CENTER + offsetMargin));
          }
  ...
  Widget _buildMovableInfoBox(DrawingElement e) {
    ...
    String info = "[$name]\n";
    if (e.type == Tool.generator) info += e.isSlack ? "V:${e.vPu}∠${e.thetaDeg}°" : "P:${e.pPu}\nV:${e.vPu}";
    else if (e.type == Tool.load) info += "P:${e.pPu}\nQ:${e.qPu}";
    else if (e.type == Tool.line) info += "${e.rPu}+j${e.xPu}";
  ...
  void _handleDrawingTap(Offset pos) {
    setState(() {
      ...
      if (selectedTool == Tool.bus) {
        ...
        // Null Safety 수정: target이 null이 아닐 때만 _getSnapPoint에 넘겨주기 위해 target! 사용
  ...
  Widget _buildBusGenLoadWidget(DrawingElement e) {
    ...
    Color baseColor = e.type == Tool.bus ? Colors.black : (e.type == Tool.generator ? (e.isSlack ? Colors.red : Colors.black) : Colors.blue);
    Color drawColor = isSelected ? Colors.blue : baseColor;
    
    Widget shapeContent;
    if (e.type == Tool.generator) {
      shapeContent = Container(width: e.width, height: e.height, decoration: BoxDecoration(color: Colors.white, border: Border.all(color: drawColor, width: 2), shape: BoxShape.circle), child: Center(child: Text("G", style: TextStyle(color: drawColor, fontWeight: FontWeight.bold, fontSize: e.height * 0.4))));
    } else if (e.type == Tool.load) {
      shapeContent = CustomPaint(size: Size(e.width, e.height), painter: TrianglePainter(fillColor: baseColor.withOpacity(0.1), strokeColor: drawColor));
      ...
      shapeContent = Stack(
        ...
        children: [
          Container(
            ...
            height: e.height, 
            decoration: BoxDecoration(
              color: e.type == Tool.bus ? baseColor : baseColor.withOpacity(0.1), 
          ...
          if (e.type == Tool.bus) 
            Positioned(
              top: -20, // 버스 사각형 살짝 위에 글씨 배치
              child: Text(
                e.label.isNotEmpty ? "Bus ${e.label}" : e.id, 
                style: TextStyle(fontWeight: FontWeight.bold, color: drawColor, fontSize: 13)
              ),
            )
        ],
      );
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                if (e.type == Tool.generator || e.type == Tool.bus) ...[
                  ...
                  if (tempIsSlack || e.type == Tool.bus) 
                    ...
                    TextField(controller: pCtrl, decoration: const InputDecoration(labelText: "출력 P (pu)")),
                ],
                if (e.type == Tool.load) ...[ 
...
class LinePainter extends CustomPainter {
  ...
  @override
  void paint(Canvas canvas, Size size) {
    final p = Paint()..color = isSelected ? Colors.blue : Colors.black..strokeWidth = isSelected ? 4 : 2..style = PaintingStyle.stroke..strokeCap = StrokeCap.round;
    Path path = Path();
...
class TransformerPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(Offset(size.width / 2, size.height / 2 - radius * 0.8), radius, paint);
    canvas.drawCircle(Offset(size.width / 2, size.height / 2 + radius * 0.8), radius, paint);
  }
