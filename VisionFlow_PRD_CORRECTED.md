...
- **PRD Version:** Competition Final — Code-Verified Revision
- **현행 코드 확인 기준일:** 2026-09-01
- **개발 기간:** 2주
- **개발 인원:** 2명
- **기존 시스템:** VisionFlow
- **핵심 원칙:** 기존 Vision / Topology / Flutter는 최대한 재사용하고, GraphDocument·검증 UI·Agent Tool Layer·pandapower 계산 계층은 신규 구현
- **주요 사용자 환경:**
...
# 3. 기존 시스템에서 유지할 것과 신규 구현할 것

아래 항목은 2026-09-01 현재 코드 기준으로 **구현됨**, **부분 구현**, **신규 구현**을 구분한다. PRD의 목표 기능과 현재 구현 상태를 혼동하지 않는다.
...
## 3.2 Power System
- **부분 구현:** `construct_y_bus(elements)`를 이용한 Y-Bus 행렬 구성
- **부분 구현:** Flutter의 `/run_simulation` 호출 인터페이스와 Excel의 Generator/Line 시트 파싱
- **신규 구현:** `build_net_from_elements()` 및 pandapower bus / generator / load / line / transformer 생성
- **신규 구현:** `run_power_flow_simulation()`과 수렴 여부, Bus/Line 결과, 계통 손실, 최소·최대 전압, 최대 선로 loading 계산

> **현행 코드 주의:** 현재 통합 FastAPI의 `/run_simulation`은 실제 pandapower 계산 결과를 반환하지 않는다. 따라서 pandapower 연동은 재사용 대상이 아니라 MVP 구현 대상이다.
...
Flutter Canvas
```
> **핵심:** Flutter Canvas로 넘어가기 전에 검수 Gate를 추가한다. `VERIFIED`는 완전무결성을 보증하는 표현이 아니라, 자동 검증에서 Critical Issue가 없고 남은 경고를 사용자가 확인·승인한 특정 revision임을 의미한다.
...
graph TD
    ...
    Step2 --> Proposal[Agent Tool 재분석 및 수정안 생성]
...
기존 Vision/CV 내부 기능을 입력·출력 계약이 고정된 Adapter로 래핑하여 Agent Tool로 노출한다. 아래 함수명은 목표 API이며, 현행 코드에 동일한 callable 함수가 모두 존재하는 것은 아니다.

- **Adapter 신규 구현:** `secondary_inference()`: 기존 2차 추론 경로를 독립 호출 가능한 Tool로 래핑
- **Adapter 신규 구현:** `high_resolution_inference()`: 기존 고해상도/타일링 경로를 Tool로 래핑
- **신규 구현:** `roi_reanalysis(bbox)`: 특정 관심 영역(ROI) 국소 재분석
- **신규 구현:** `port_aware_retry(node_id)`: 포트 중심 결선 스켈레톤 재추적
- **신규 구현:** `merge_fragmented_bus(bus_ids)`: 분절된 버스 병합 연산
- **부분 구현 → Adapter 필요:** `validate_topology(graph_doc)`: 기존 `validate_graph(nodes, lines)` 규칙을 GraphDocument Tool로 확장

Agent Tool은 결과를 직접 확정하지 않고 `proposed_patch`, `evidence`, `confidence`, `affected_ids`를 반환한다. LLM은 픽셀 좌표나 결선을 직접 작성할 수 없으며, Deterministic Tool이 만든 변경안만 선택·설명할 수 있다.
...
```
> **가드레일:** 무한 루프 방지를 위해 자동 Retry는 `MAX_ATTEMPTS = 2~3`으로 제한한다. 재분석 결과가 기존 GraphDocument를 변경하는 경우 변경 전·후 Preview와 revision 이력을 남기며, 토폴로지 변경은 사용자 승인 또는 명시된 고정밀 자동 적용 기준을 통과해야 한다.
...
Human Knowledge (User Intent)
        ↓
Final Verification (사용자 승인된 디지털 모델 revision)
```
...
- `All Components Reviewed (PASS)`
- `Topology Validation Executed (PASS)`
- 남은 Warning이 있다면 사용자가 각각 확인하고 승인

사용자가 **`[ 🚀 회로도 검증 완료 ]`**를 클릭하면 해당 `revision`만 `status = VERIFIED`로 전환한다. 이후 노드·포트·선로가 수정되면 즉시 새 revision을 만들고 상태를 `IN_REVIEW`로 되돌린다.
...
GraphDocument:
  ...
  audit_log: list[ChangeOperation]
```
...
Node:
  ...
  bbox:                     # 현재 API의 [center_x, center_y, width, height]를 명시적 필드로 변환
    center_x: float
    center_y: float
    width: float
    height: float
  original_bbox:            # 최초 검출 좌표 보존, bbox와 동일한 center-xywh 형식
    center_x: float
    center_y: float
    width: float
    height: float
...
> **좌표 규칙:** 원본 이미지의 좌상단을 `(0, 0)`으로 사용한다. Node bbox의 위치는 심볼 중심점 `center_x`, `center_y`이고 크기는 `width`, `height`다. Line path의 각 점은 `{x, y}` 형식으로 직렬화하며 Flutter Canvas 오프셋은 화면 Adapter에서만 적용한다.
...
Agent가 수정이 필요하다고 판단해도 GraphDocument를 직접 덮어쓰지 않는다. 반드시 등록된 Tool이 구조화된 `proposed_patch`를 만들고, 스키마 검증·토폴로지 재검증·Preview·감사 로그 기록을 거쳐 적용한다.
...
| **Day 9** | pandapower 최소 네트워크 생성기 및 조류계산 신규 구현 | 수동 파라미터 입력값 검증 UI | **Gate: 고정 테스트 계통의 조류계산 수렴** |
| **Day 10** | Excel Parser 개선, Bus 번호 매칭, Power Flow Readiness 검증 | Excel 업로드 UI & 매핑 현황 표시 | 데이터 매핑 완료 |
| **Day 11** | 핵심 Power Query Tools (Bus V, Line P/Q, Loading, Loss) | 자연어 질의 입력창 & 결과 표시 카드 | 핵심 쿼리 어댑터 완성 |
| **Day 12** | 단일 GPT Orchestrator (제한된 Tool Calling) | Agent 대화형 해석 Assistant UI | 자연어 계통 해석 |
...
3. **Calculation Core**: GraphDocument 기반 pandapower 네트워크 생성, 조류계산 수렴 확인, Bus 전압·Line P/Q·Loading·Loss 조회
4. **Data Mapping**: Excel 업로드, Bus 번호 기반 매핑, 누락·중복·형식 오류 검증
5. **Agent MVP**: Review Issue 설명, ROI/Port Retry Tool 선택, 핵심 계산 결과 질의응답. LLM이 좌표·결선·수치를 직접 생성하지 않았음을 Tool 실행 로그로 확인
...
- 선로 조류 애니메이션 화살표
- 모선 전압 히트맵
- 파라미터 변경 전·후 What-If 비교
- 모든 설비 유형을 포괄하는 자유형 자연어 질의
