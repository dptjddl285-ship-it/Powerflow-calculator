...
class OpenAIReviewAssistantProvider(ReviewAssistantProvider):
    ...
    def generate_proactive_summary(
        ...
        # Fallback to local structured summary logic for efficiency & cost protection
    ...
    def answer_chat(
        ...
        # Enforce Chat History limit (Cost Safety)
        hist = history or []
...
def get_assistant_provider() -> ReviewAssistantProvider:
    ...
    - Default is ALWAYS `LocalReviewAssistantProvider` (free, fast, deterministic).
    - `OpenAIReviewAssistantProvider` is only activated when explicitly requested via:
      `AI_PROVIDER=openai` AND `OPENAI_API_KEY` is present.
    - If `OPENAI_API_KEY` exists but `AI_PROVIDER` is unset or 'local', Local provider is used.
    """
    configured_provider = os.environ.get("AI_PROVIDER", "local").strip().lower()
    openai_api_key = os.environ.get("OPENAI_API_KEY", "").strip()

    if configured_provider == "openai" and openai_api_key:
        model_name = os.environ.get("OPENAI_MODEL", "gpt-4o")
        return OpenAIReviewAssistantProvider(api_key=openai_api_key, model_name=model_name)
