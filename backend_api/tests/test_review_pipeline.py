...
class ReviewPipelineTest(unittest.TestCase):
    ...
    def test_t_chat_reviewer_context_and_fallback(self):
        ...
        from agent import chat_reviewer
        res_summary = chat_reviewer.chat(
    ...
    def test_v_default_local_provider_architecture(self):
        """Test V: Review Assistant Provider defaults to LocalReviewAssistantProvider even if OPENAI_API_KEY is present."""
        import os
        from agent.providers import get_assistant_provider, LocalReviewAssistantProvider, OpenAIReviewAssistantProvider

        # 1. Default without env vars
        ...
        try:
            ...
            # 2. OPENAI_API_KEY is set, but AI_PROVIDER is not 'openai' -> Still Local!
            ...
            self.assertEqual(prov2.provider_name, "local")

            # 3. Explicit opt-in: AI_PROVIDER=openai and OPENAI_API_KEY -> OpenAI Provider
            os.environ["AI_PROVIDER"] = "openai"
            prov3 = get_assistant_provider()
            self.assertIsInstance(prov3, OpenAIReviewAssistantProvider)
            self.assertEqual(prov3.provider_name, "openai")
        finally:
