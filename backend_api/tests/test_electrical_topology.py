...
class ElectricalTopologyTest(unittest.TestCase):
    ...
    def test_two_nearby_shallow_crossings_do_not_swap_lanes(self) -> None:
        """Each split X must pair with its local half, not the next X."""
        rows = [
            "...........###..........................................",
            ".............###........................................",
            "...............###......................................",
        ...
        skeleton = np.asarray(
            [[255 if value == "#" else 0 for value in row] for row in rows],
            dtype=np.uint8,
        )
        routes = _shortest_directional_routes(
            skeleton,
            (11, 0),
            {(55, 22), (55, 3), (0, 35), (0, 19), (36, 40)},
        )
        self.assertEqual(set(routes), {(55, 22)})

        lower_routes = _shortest_directional_routes(
            skeleton,
            (0, 19),
            {(36, 40), (55, 3), (0, 35), (11, 0), (55, 22)},
        )
        self.assertEqual(set(lower_routes), {(36, 40)})
    ...
    def test_short_source_pixel_transformer_lead_is_kept(self) -> None:
        skeleton = np.zeros((25, 25), np.uint8)
        skeleton[12, 7:18] = 255
        endpoints = {
            (7, 12): "transformer_1",
            (17, 12): "bus_1",
        }
        ports = {
            (7, 12): port("transformer_1", "right"),
            (17, 12): port("bus_1", "boundary"),
        }
        ...
        lines = trace_electrical_connections(
            ...
            endpoints,
            ports,
            {"transformer_1": "transformer", "bus_1": "bus"},
        )

        self.assertEqual(len(lines), 1)
        self.assertEqual(
            set(lines[0]["connected_to"]),
            {"transformer_1", "bus_1"},
        )
